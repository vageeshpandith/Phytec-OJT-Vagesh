...
void mqtt_data_send(char *data){
	char buffer[50];
	sprintf(&buffer[0],"CMD+MQTTPUB=reading/relay,%s\r\n", data);
...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
        ...
        HAL_GPIO_WritePin(RELAIS_PORT, RELAIS_PIN, GPIO_PIN_SET);
       mqtt_data_send("ON");// Turn ON the relay
        HAL_Delay(DELAY_TIME_MS);
        HAL_GPIO_WritePin(LED_PORT, LED_PIN, GPIO_PIN_RESET);     // Turn OFF the LED
        HAL_GPIO_WritePin(RELAIS_PORT, RELAIS_PIN, GPIO_PIN_RESET);
        mqtt_data_send("OFF");// Turn OFF the relay
        HAL_Delay(DELAY_TIME_MS);
  }
  /* USER CODE END 3 */
}
...
void MQTT_Init()
{
	...
	/*********CMD+MQTTCONCFG---->LED **********/
	//memset(&buffer[0],0x00,strlen(buffer));
	sprintf (&buffer[0], "CMD+MQTTCONCFG=3,mqtt-vageshradhya-oawdww,,,,,,,,,\r\n");
	HAL_UART_Transmit(&huart1, (uint8_t*)buffer, strlen(buffer), 1000);
	HAL_UART_Transmit(&huart2, (uint8_t*)buffer, strlen(buffer), 1000);
	//memset(&buffer[0],0x00,strlen(buffer));
	//HAL_Delay(500);
